...
项目使用 GitHub Actions 作为 CI 平台，每次推送和 PR 自动执行：

| 阶段 | 操作 | 触发时机 |
|------|------|----------|
| 测试 | `pytest tests/ -q` | push / pull_request |
| Lint | `python -m skill_library.quality.lint skills/*` | push / pull_request |
| 质量门禁 | 全部测试通过 + lint score >= 90 | push / pull_request |

**门禁原则**：CI 未通过不得合并 PR。提交前本地也应执行检查清单（见 CLAUDE.md § 提交前检查清单）。

### 13.2 版本管理策略

统一版本管理使用 `bump-my-version`，版本格式为语义化版本 `MAJOR.MINOR.PATCH`：

| 场景 | 操作 | 示例 |
|------|------|------|
| Epic 完成 | `bump-my-version bump minor` | 0.1.0 → 0.2.0 |
| Bugfix / 小调整 | `bump-my-version bump patch` | 0.1.0 → 0.1.1 |
| 重大里程碑 | `bump-my-version bump major` | 1.0.0 → 2.0.0 |

**版本源文件**（同步更新）：

- `pyproject.toml` → `project.version`
- `src/skill_library/__init__.py` → `__version__`
- `docs-alignment.json` → `project-version`

### 13.3 自动发布（Release）

当版本标签（`v*`）被推送到 GitHub 时，自动触发 Release 流程：

```
version bump commit + tag → git push --tags → GitHub Actions Release
                                                   ↓
                                           GitHub Release 创建
                                           ↓              ↓
                                      Changelog 生成     PyPI 发布（可选）
```

**Changelog 生成**：基于版本标签间的 commit 历史自动生成。

### 13.4 升级渠道

提供两条升级路径：

**路径一：手动脚本（human-first）**

`scripts/upgrade.sh` / `scripts/upgrade.ps1` 执行：
1. 检查当前版本（`git describe --tags`）
2. 查询 GitHub 最新 Release
3. 有更新则 `git pull` + 重装依赖

**路径二：skill-manager 升级工作流（agent-first）**

skill-manager 的 SKILL.md 包含升级指导，AI Agent 可自主执行：
1. 读取当前版本标签
2. 通过 GitHub API 查询最新 Release
3. 比较版本，有更新时通知用户或自动执行升级
...
当前所有 skill 共享项目版本号，无法按能力域独立发布。Agent 需要：
- 安装特定版本的 `development` 包，而非整个库
- 明确每个包是否兼容当前环境
- 按需增量更新单个包，不干扰其他包

### 14.2 Pack 版本结构

每个技能包在 `skills/<pack>/pack.json` 中声明独立版本：

```json
{
  "name": "development",
  "version": "1.0.0",
  "description": "编码、调试、测试相关 skill",
  "skills": ["code-lint", "test-run"],
  "dependencies": [],
  "updated": "2026-05-23"
}
```
...
| 技能包 | 版本 | 说明 | Skill 数 |
|--------|------|------|----------|
| meta | v1.0.0 | 管理其他 skill | 3 |
| retrieval | v0.1.0 | 知识库、文件搜索 | 0 |
| web | v0.1.0 | 联网搜索、抓取 | 0 |
| document | v0.1.0 | 文档读写、转换 | 0 |
| development | v0.1.0 | 编码、调试、测试 | 0 |
| devops | v0.1.0 | 部署、CI/CD | 0 |
| communication | v0.1.0 | 消息、邮件 | 0 |
| learning | v0.1.0 | 教学、知识沉淀 | 0 |
| security | v0.1.0 | 安全扫描、漏洞检测 | 0 |
| data | v0.1.0 | 数据处理、ETL | 0 |

### 14.4 包版本独立发布

GitHub Release 时包含每包的 `pack.json` 版本信息和 artifact 清单：

- Release 正文列出所有包及其版本
- Agent 可解析 Release 信息获取特定包的最新版本
- 未来可演进为独立包下载（tar/zip artifact）
