...
> 版本：1.1.0 | 更新：2026-05-23

跨项目 Agent Skill 管理系统。**Skill 即接口** — 通过 meta-skill SKILL.md 管理技能全生命周期，CLI 是选配。
...
### 主路径（Any Agent，无 CLI 必需）

```bash
# 将元 skill 复制到 Agent 技能目录即可使用
...
Agent 自动识别 SKILL.md，按指引执行管理操作。仅 lint 需要 Python 3.11+。
...
所有命令支持 `--help`。效果与 skill-based 操作一致。
...
**16 个 Epic，79 个 Story，345 个测试 — 全部通过。**
